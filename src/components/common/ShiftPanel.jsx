...
//     () =>
//       left.filter((i) =>
//         (i.label ?? "").toLowerCase().includes(qLeft.toLowerCase())
//       ),
//     [left, qLeft]
//   );
...
//     () =>
//       right.filter((i) =>
//         (i.label ?? "").toLowerCase().includes(qRight.toLowerCase())
//       ),
//     [right, qRight]
//   );
...
export default function ShiftPanel({
  ...
  const toggleSelectAll = useCallback(
    (visible, selectedIds, setSelectedIds) => {
      const allVisibleSelected = visible.every((i) =>
        selectedIds.includes(i.id)
      );

      if (allVisibleSelected) {
        setSelectedIds(
          selectedIds.filter((id) => !visible.some((i) => i.id === id))
        );
      } else {
        const newIds = [
          ...selectedIds,
  ...
  const moveSelected = (from) => {
    if (from === "left") {
      if (!leftSelectedIds.length) return;

      const moving = left.filter((i) => leftSelectedIds.includes(i.id));
      const newLeft = left.filter((i) => !leftSelectedIds.includes(i.id));
      const newRight = [...right, ...moving];
      ...
      if (!rightSelectedIds.length) return;

      const moving = right.filter((i) => rightSelectedIds.includes(i.id));
      const newRight = right.filter((i) => !rightSelectedIds.includes(i.id));
      const newLeft = [...left, ...moving];
  ...
  // ⭐ Memoized list item to stop scroll jumping
  const Item = memo(function Item({ item, selectedIds, setSelectedIds }) {
    return (
      <li key={item.id}>
        <label className="flex items-center gap-3 rounded-lg px-3 py-2 cursor-pointer hover:bg-gray-50 border border-transparent hover:border-gray-200">
          <input
            type="checkbox"
            checked={selectedIds.includes(item.id)}
            onChange={() => toggleMulti(setSelectedIds)(item.id)}
            className="h-4 w-4 accent-blue-600"
          />
          <span className="text-sm text-gray-800">{item.label}</span>
        </label>
  ...
  // ⭐ Memoized Panel to stop cursor jumping
  const Panel = useCallback(
      ...
      return (
        <div className="flex flex-col rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          <div className="flex items-center justify-between p-3 border-b bg-gray-50">
            ...
            <button
              className="text-xs rounded-md px-2 py-1 border bg-white hover:bg-gray-100"
              onClick={() =>
                toggleSelectAll(visible, selectedIds, setSelectedIds)
              }
            >
              {visible.every((i) => selectedIds.includes(i.id))
                ? "Unselect All"
                : "Select All"}
            </button>
          </div>
          ...
          <div className="h-64 overflow-auto px-2 pb-2">
            {visible.length === 0 ? (
    ...
    [toggleSelectAll, toggleMulti]
  );

  return (
    <div className="w-full">
      <div className="grid gap-4 grid-cols-1 md:grid-cols-[1fr_auto_1fr] items-stretch">
        <Panel
          title={titleLeft}
          items={left}
          selectedIds={leftSelectedIds}
          setSelectedIds={setLeftSelectedIds}
          query={qLeft}
          setQuery={setQLeft}
          visible={filteredLeft}
        />
        ...
        <div className="flex md:flex-col gap-2 justify-center items-center">
          <div className="flex gap-2">
            <button
              onClick={() => moveSelected("left")}
              className="px-3 py-2 rounded-lg border bg-white hover:bg-gray-50 shadow-sm"
            >
              &gt;
            </button>
            <button
        ...
        <Panel
          ...
          setSelectedIds={setRightSelectedIds}
          query={qRight}
          setQuery={setQRight}
          visible={filteredRight}
        />
